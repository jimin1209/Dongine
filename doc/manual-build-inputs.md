...
| Google Calendar OAuth 설정 | Google Cloud Console(동일 Firebase 프로젝트 권장) | 캘린더 연동 | [§2-7](#google-calendar-oauth): Calendar API 사용 설정 → 동의 화면(테스트 사용자) → Android(SHA·패키지명) → iOS(번들 ID) |
...
현재 `android/app/build.gradle.kts`는 release 빌드에서도 debug signing을 쓰도록 되어 있다.
...
6. Android release signing(`key.properties`·keystore)
7. iOS 서명·배포 자격
