...
<a id="readme-demo-vs-deploy-status"></a>

## 데모 vs 배포 · 완료/남음 한눈에

**한 문서 축**: 데모 가능 조건과 스토어·운영 배포 가능 조건의 차이·수동 체크 전체 목록은 **[doc/manual-build-inputs.md](./doc/manual-build-inputs.md)**에 모아 두었다([데모 vs 배포 표](./doc/manual-build-inputs.md#demo-vs-deploy-conditions), [수동 체크 최종 정리본](./doc/manual-build-inputs.md#manual-check-final-summary)).

| 관점 | 무엇이 이미 있다 | 무엇을 문서·게이트로 닫는다 | 더 읽을 곳 |
|------|------------------|---------------------------|------------|
| **코드·기능** | 아래 [시제품 데모 가능 기능](#readme-demo-capable-features)에 구현된 범위 | — | [doc/test-strategy.md](./doc/test-strategy.md) |
| **데모(시제품 시연)** | Debug 빌드·시드 UI로 시나리오 가능 | [release-checklist.md](./doc/release-checklist.md) §0~§6, smoke·워크스루(표 1~9) | [doc/demo-operator-checklist.md](./doc/demo-operator-checklist.md)(당일 타임라인), [doc/demo-smoke-push-map-calendar.md](./doc/demo-smoke-push-map-calendar.md), [doc/demo-walkthrough.md](./doc/demo-walkthrough.md) |
| **실제 배포·운영** | 위 데모 게이트를 전제로 확장 | 릴리스 서명·스토어 파이프라인·규칙 최종 검토·관측(선택 기능 포함) | [release-checklist.md](./doc/release-checklist.md) §5~§6·§8, [manual-build-inputs.md](./doc/manual-build-inputs.md) §2 |
| **남은 제품 작업·백로그** | — | UX·빈 상태·검증 문서 등 | [doc/prototype-remaining-work.md](./doc/prototype-remaining-work.md) |

**빌드·배포 전 사람 손 목록(중복 없이 한곳)**: [manual-build-inputs — 수동 체크 최종 정리본](./doc/manual-build-inputs.md#manual-check-final-summary).

---

## 시제품 데모 준비 — 문서 진입 경로

<a id="readme-demo-operator-checklist"></a>

**시연 당당 초압축 체크리스트**: [doc/demo-operator-checklist.md](./doc/demo-operator-checklist.md) — **10분 전 / 2분 전 / 직전 / 시연 중**만 틱으로 정리. 푸시·지도 **절차**는 [smoke](./doc/demo-smoke-push-map-calendar.md), **3–5분 대본**은 [워크스루](./doc/demo-walkthrough.md)로 연결한다.
...
| **실기기 검증** | 6 | [doc/real-device-validation-matrix.md](./doc/real-device-validation-matrix.md) | **QA·리허설용** 기능별 P/F 표(§9 푸시·딥링크 포함). 체크리스트와 항목·깊이가 다르며, 위치 백그라운드는 [location-background-demo.md](./doc/location-background-demo.md)와 §8을 함께 본다 |
| **데모 당일** | 7 | [doc/demo-smoke-push-map-calendar.md](./doc/demo-smoke-push-map-calendar.md) | **커튼 직전** 약 1–2분 — 푸시·지도·(선택) Google Calendar만 “살았는지” 재확인 |
...
**푸시 알림 탭 → 기대 화면**(검증·시연 공통 기준)은 **[doc/demo-smoke-push-map-calendar.md — 1. 푸시](./doc/demo-smoke-push-map-calendar.md#smoke-push)** 표 **한 곳**에만 적어 두었다. `data.route` 불일치·허용 목록 밖 동작(디버그 로그 문구 포함)은 [push-route-debug](./doc/demo-smoke-push-map-calendar.md#push-route-debug)를 본다.
...
| 문서 | 용도 |
|------|------|
| [doc/demo-operator-checklist.md](./doc/demo-operator-checklist.md) | 시연 당당 **당일 타임라인**(10분 전·2분 전·직전·시연 중)·fallback — 상세는 smoke·워크스루로 연결 |
| [doc/location-background-demo.md](./doc/location-background-demo.md) | 백그라운드 위치 **시연 설명 vs 기술 한계**·Android/iOS 권한·배너·수동 점검 단계 |
...
<a id="readme-demo-capable-features"></a>
...
**시연 대본과 한계를 나누어 쓰려면** [doc/location-background-demo.md](./doc/location-background-demo.md)를 본다(Android/iOS **단계별 권한·배너·알림**·「앱 살아 있음」vs **완전 종료**).
...
| 구분 | 내용 |
|------|------|
| **시연용(짧게)** | [demo-walkthrough.md](./doc/demo-walkthrough.md) 9단계: 지도 탭 → **배너·토글 라벨** → 공유 ON → 내 마커(가능하면 2대). 「가족 지도 + 공유 UI」 설명에 충분하다. |
| **기술 한계(반드시 구분)** | 갱신은 **앱 프로세스가 살아 있을 때만**. **완전 종료·OS 종료** 시 멈춘다. iOS 백그라운드 스트림은 **`always` 권한** 전제. 상세 표는 [location-background-demo.md](./doc/location-background-demo.md). |
| **수동 검증** | [real-device-validation-matrix.md](./doc/real-device-validation-matrix.md) §8 — 배너·권한은 **8-5**, 홈으로 보낸 뒤 갱신은 **8-6**(30초 이상, iOS는 **항상** 허용 전제). |

### 가족 위치 화면(`LocationScreen`)

상단에 기기 위치 서비스·앱 위치 권한 상태를 짧게 표시한다. 백그라운드 공유에 불리한 상태(특히 iOS **앱 사용 중만**)면 설명과 함께 `Geolocator.openAppSettings` / `openLocationSettings`로 이어지는 버튼을 둔다. 공유 토글은 Firestore와 동기화되지만, 권한이 부족하면 「켜짐(위치 불가)」「공유 중(백그라운드 제한)」처럼 실제 갱신 가능 여부를 라벨로 구분한다. 지도 하단 안내 문구는 **프로세스 생존·알림·완전 종료 시 중단**을 직접 적어 두었으므로, 시연 시 **말로 과장하지 않도록** 이 문구와 [location-background-demo.md](./doc/location-background-demo.md)의 「시연 vs 한계」를 맞춘다.
...
- **탭 시 기대 화면**은 [smoke 문서 표](./doc/demo-smoke-push-map-calendar.md#smoke-push)와 동일. **서버·앱 route 정의·불일치 시 점검**은 [push-route-debug](./doc/demo-smoke-push-map-calendar.md#push-route-debug)를 본다.
...
**당일 축약**: 표 **1~6** 완료 후 → **7** [smoke](./doc/demo-smoke-push-map-calendar.md)(푸시·지도·선택 Google) → **8** Debug `flutter run` → **홈** 앱바 톱니 → **설정** (`/settings`) 하단에서 데모 **초기화·채우기** → **9** [워크스루](./doc/demo-walkthrough.md). **담당자용 시간축만**은 [운영 체크리스트](./doc/demo-operator-checklist.md); 진입은 [위 단락](#readme-demo-operator-checklist) 또는 [데모 준비 표](#시제품-데모-준비--문서-진입-경로) 직후 안내.
